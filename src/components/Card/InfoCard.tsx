...
import error_img from "@/assets/free_throws.png";
import type { InfoCardProps } from "@/types/cards.types";

const InfoCard = ({ data, type = "noticias", index = 0 }: InfoCardProps) => {
  const formattedDate = formatDate(data.datas?.[0]) || "Data a definir";
  const imageUrl = !data.image || data.image === "" ? error_img : data.image;
    ...
    >
      <div className="relative h-56 sm:h-64 md:h-72 overflow-hidden">
        ...
        <div className="absolute top-4 left-4">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-white shadow-lg backdrop-blur-sm bg-gradient-to-r from-[#162456]/90 to-blue-600/90">
      ...
      <div className="p-4 sm:p-6 flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100 mt-4">
          <Link
            href={`/${type}/${data.id}`}
            className={`group/btn relative inline-flex items-center gap-2 px-4 py-2.5 bg-gradient-to-r from-[#162456] to-blue-600 hover:from-blue-600 hover:to-[#162456]
