...
export default function NoticiasDetailPage() {
  ...
  const noticiaId = params.id as string; // O ID do noticia da URL
  const [noticia, setNoticia] = useState<noticiaData>();
  ...
  useEffect(() => {
    async function fetchCompeticoes() {
      try {
        ...
        const data: noticiaData[] = await response.json();
        const noticiaData = data.find((team) => team.id === noticiaId);
        if (!noticiaData) {
  ...
  if (!noticia) {
    return (
      <div className="flex justify-center items-center min-h-screen bg-gray-100 text-gray-700 p-4">
        <p className="text-xl">noticia não encontrado.</p>
      </div>
    );
  }
